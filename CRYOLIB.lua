...
function CryoLib:CreateWindow(hubName)
    ...
    -- Drag floatBtn dengan tween mulus
    local fDrag, fS, fP
    local fTweenTarget = nil
    ...
    UserInputService.InputChanged:Connect(function(i)
        if fDrag and i.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local targetPos = UDim2.new(fP.X.Scale, fP.X.Offset + d.X, fP.Y.Scale, fP.Y.Offset + d.Y)
            TweenService:Create(floatBtn, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Position = targetPos
            }):Play()
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(i) 
        if dDrag and i.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local targetPos = UDim2.new(dS.X.Scale, dS.X.Offset + d.X, dS.Y.Scale, dS.Y.Offset + d.Y)
            TweenService:Create(mainPanel, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Position = targetPos
            }):Play()
    ...
    function libData:CreateTab(name)
        ...
        function tabObj:CreateDropdown(txt, options, callback)
            local isOpen = false
            local selected = options[1] or "Select..."

            -- Container utama (collapsed = 48px)
            ...
            titleLabel.Size = UDim2.new(1, -60, 0, 48)
            ...
            local itemHeight = 34
            local menuHeight = (#options * (itemHeight + 2)) + 8
            ...
            for idx, opt in ipairs(options) do
                ...
                item.MouseLeave:Connect(function()
                    ...
                    TweenService:Create(item, TweenInfo.new(0.15), {
                        ...
                        TextColor3 = isSelected and libData.accent or Color3.fromRGB(200, 200, 215)
                    }):Play()
                end)
                item.MouseButton1Click:Connect(function()
                    ...
                    -- Tutup menu dengan tween
                    isOpen = false
                    TweenService:Create(menu, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            ...
            toggleBtn.MouseButton1Click:Connect(function()
                ...
                if isOpen then
                    ...
                    TweenService:Create(container, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                        Size = UDim2.new(1, -4, 0, 48 + menuHeight + 6)
                    }):Play()
                    TweenService:Create(arrow, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {
                        TextColor3 = libData.accent,
                        Rotation = 180
                    ...
                    TweenService:Create(menu, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                        Size = UDim2.new(1, 0, 0, 0)
                    }):Play()
                    TweenService:Create(container, TweenInfo.new(0.22, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                        Size = UDim2.new(1, -4, 0, 48)
                    }):Play()
                    TweenService:Create(arrow, TweenInfo.new(0.22, Enum.EasingStyle.Quart), {
                        TextColor3 = Color3.fromRGB(130, 130, 150),
                        Rotation = 0
                    }):Play()
